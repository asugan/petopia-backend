...
// Unique index to prevent race conditions when multiple scheduler instances
// try to create the same notification simultaneously.
// Only applies to 'sent' status to allow retries for failed notifications.
scheduledNotificationSchema.index(
  { eventId: 1, notificationId: 1 },
  { unique: true, partialFilterExpression: { status: 'sent' } }
);

export const ScheduledNotificationModel = model<IScheduledNotificationDocument>(
